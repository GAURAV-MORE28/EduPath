...
def quality_metrics(cases_by_area: dict[str, list[dict[str, Any]]]) -> dict[str, dict[str, Any]]:
    ...
    if p:
        ...
        put("planner", "mean budget utilization", _rate(sum(_m(p, "utilization")), len(_m(p, "utilization"))), len(p), "KNOWN LIMITATION (thin plans): measured, not fixed in this stage")
